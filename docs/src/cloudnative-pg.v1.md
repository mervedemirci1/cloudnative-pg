...
| `walRestoreRetryTimeout` _[Duration](https://kubernetes.io/docs/reference/generated/kubernetes-api/v1.34/#duration-v1-meta)_ | WalRestoreRetryTimeout bounds how long a single invocation of<br />`manager wal-restore` retries transient failures (network blips,<br />flaky object stores, plugin glitches) before asking PostgreSQL to<br />stop log-shipping replication.<br />Without this bound, a transient failure during a replica promotion<br />can cause the replica to promote on a partial archive, losing<br />already-archived transactions.<br />Must be positive. Defaults to 5 minutes when unset. |  |  |  |
